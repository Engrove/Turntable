...
// KORRIGERING: Använder '||' för default-värden.
...
// KORRIGERING: Använder '||' för logisk OR.
...
<!-- Stora referensgeometrier (skivkanter och svepbåge) --> 
  <path :d="`M ${pivot.x - effectiveLength} 0 A ${effectiveLength} ${effectiveLength} 0 0 1 ${pivot.x} ${-effectiveLength}`" class="arc-path" /> <circle :cx="spindle.x" :cy="spindle.y" r="146.05" class="record-edge" /> <circle :cx="spindle.x" :cy="spindle.y" r="60.325" class="record-edge" /> <!-- Tonarmens delar --> <line :x1="pivot.x" :y1="pivot.y" :x2="stylus.x" :y2="stylus.y" class="tonearm-line" /> <circle class="spindle-point" :cx="spindle.x" :cy="spindle.y" r="3.5" /> <circle class="pivot-point" :cx="pivot.x" :cy="pivot.y" r="3.5" /> <circle class="stylus-point" :cx="stylus.x" :cy="stylus.y" r="2" /> <!-- Nollpunkter --> <g v-if="innerNullCoord" class="null-point inner"> <circle :cx="innerNullCoord.x" :cy="innerNullCoord.y" r="3" /> <text :x="innerNullCoord.x + 8" :y="innerNullCoord.y">Inner Null</text> </g> <g v-if="outerNullCoord" class="null-point outer"> <circle :cx="outerNullCoord.x" :cy="outerNullCoord.y" r="3" /> <text :x="outerNullCoord.x + 8" :y="outerNullCoord.y">Outer Null</text> </g> <!-- Måttlinjer och etiketter --> <g class="dimension-line p2s"> <line :x1="spindle.x" y1="30" :x2="pivot.x" y2="30" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)" /> <text :x="pivotToSpindle / 2" y="22">Pivot-to-Spindle: {{ pivotToSpindle.toFixed(1) }}mm</text> </g> <g class="dimension-line overhang"> <line :x1="spindle.x" y1="-30" :x2="stylus.x" y2="-30" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)" /> <text :x="overhang / -2" y="-38">Overhang: {{ overhang.toFixed(1) }}mm</text> </g> <g class="dimension-line effective-length"> <line :x1="pivot.x" y1="55" :x2="stylus.x" y2="55" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)" /> <text :x="(pivot.x + stylus.x) / 2" y="68">Effective Length: {{ effectiveLength.toFixed(1) }}mm</text> </g> </svg> </div> 
