...
// NYTT & KORREKT: computed-egenskap för att hämta info om vald standard.
...
<div class="preset-group"> <label>Load Tonearm Preset (Optional)</label> <div class="preset-selectors"> <select v-model="selectedTonearmManufacturer" class="value-select manufacturer"> <option :value="null" disabled>Select Manufacturer</option> <option v-for="man in tonearmManufacturers" :key="man" :value="man">{{ man }}</option> </select> <select v-model="store.selectedTonearmId" @change="store.loadTonearmPreset($event.target.value)" :disabled="!selectedTonearmManufacturer" class="value-select model"> <option :value="null" disabled>Select Model</option> <option v-for="arm in filteredTonearms" :key="arm.id" :value="arm.id">{{ arm.model }}</option> </select> <button v-if="store.selectedTonearmId" @click="resetTonearmSelection" class="reset-preset-btn" title="Clear tonearm selection">×</button> </div> </div> <div class="input-group"> <label for="p2s">Pivot-to-Spindle Distance (mm)</label> <div class="input-control"> <input type="range" id="p2s" min="150" max="400" step="0.1" v-model.number="store.userInput.pivotToSpindle" @input="store.calculateAlignment" :disabled="!isPivotingArm"> <input type="number" class="value-display" step="0.1" v-model.number="store.userInput.pivotToSpindle" @change="store.calculateAlignment" :disabled="!isPivotingArm"> </div> </div> 
...
<div class="mode-switch"> <button v-for="(geo, key) in store.ALIGNMENT_GEOMETRIES" :key="key" :class="{ active: store.userInput.alignmentType === key }" @click="store.setAlignment(key)" :title="geo.description" :disabled="!isPivotingArm" > {{ key }} </button> </div> <div v-if="isPivotingArm" class="geometry-info"> <strong>{{ store.calculatedValues.geometryName }}:</strong> <span>{{ store.calculatedValues.geometryDescription }}</span> </div> <div v-else class="non-pivoting-info"> <strong>{{ store.calculatedValues.geometryName }} Arm Detected</strong> <span>This is a tangential tracking tonearm. Standard alignment geometries do not apply.</span> </div> 
...
<!-- NYTT & KORREKT: Informationsruta för Groove Standard -->
