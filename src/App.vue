...
/**
* @file src/App.vue
* @description Applikationens rotkomponent. Hanterar huvudlayouten,
* sidomenyn, mobilanpassning och globala banners.
*/
...
const bannerClass = computed(() => bannerState.value === 'none' ? 'banner-hidden' : '');

const toggleMenu = () => { isMenuExpanded.value = !isMenuExpanded.value; };
const closeMenu = () => { isMenuExpanded.value = false; };
...
setTimeout(() => { bannerState.value = 'updated'; }, thirtyMinutesInMillis - timeSinceDeploy);
setTimeout(() => { bannerState.value = 'none'; }, sixtyMinutesInMillis - timeSinceDeploy);
...
<p>
<strong>Site Has Moved!</strong> This version is now an archive. For the latest updates, please update your bookmarks to our new address: <a href="https://engrove.pages.dev" target="_blank" rel="noopener noreferrer">engrove.pages.dev</a>
</p>
</div>
...
<nav class="main-nav">
<RouterLink v-for="route in router.options.routes.filter(r => r.meta && r.meta.title && !r.meta.isReportPage)" :key="route.name" :to="route.path" class="nav-link" :title="isMenuExpanded ? '' : route.meta.title" @click="isMobile && closeMenu()">
...
.migration-banner { position: sticky; top: 0; left: 0; width: 100%; background-color: #34495e; color: white; z-index: 2001; padding: 0.75rem 1rem; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
.migration-banner .banner-content p { margin: 0; font-weight: 500; }
.migration-banner .banner-content a { color: #a9cce3; font-weight: 700; text-decoration: underline; }
.close-banner-btn { background: none; border: none; color: white; font-size: 1.5rem; font-weight: bold; cursor: pointer; padding: 0 0.5rem; line-height: 1; opacity: 0.7; transition: opacity 0.2s ease; }
