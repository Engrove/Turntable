...
// --- STATE (Alla justerbara parametrar) ---
const params = ref({
m_headshell: 11.4,
m_pickup: 6.3,
m_screws: 1.3,
...
compliance: 10.0,
    ...
    // Dessa är nu fullt interaktiva
    m_tube_percentage: 35.0,
    L2: 15.0, // Pivot to armwand's CoG
    L3_fixed_cw: 12.5, // Pivot to fixed counterweight's CoG
});

// --- COMPUTED PROPERTIES (Beräknade värden som reaktivt uppdateras) ---
...
const diagnosis = computed(() => {
    ...
    if (freq >= 8.0 && freq <= 11.0) {
        return { text: 'CONGRATULATIONS! The system is very well matched. Resonance is in the ideal range (8-11 Hz).', className: 'ideal' };
    }
    if (freq >= 7.0 && freq < 8.0 || freq > 11.0 && freq <= 12.0) {
        return { text: 'ACCEPTABLE. The system is slightly outside the ideal range, but may perform well.', className: 'warning' };
    }
...
return { params, m1, m2_tube, m3_fixed_cw, calculatedResults, diagnosis };
});
